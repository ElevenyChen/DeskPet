...
- **翻肚皮** → 随机从 idle 触发，猫咪翻身露肚皮，~15s 后恢复（仅当 Sprites/belly_up/ 有素材时触发）
- **舔毛** → 随机从 idle 触发，舔爪子/舔毛，~20s 后恢复（仅当 Sprites/grooming/ 有素材时触发）
- **提醒** → 像素猫提醒动画
...
### 工作状态系统（state-first，2026-08 重构）
- **打卡上班 → 自动计时**，不再需要开计时器证明"在工作"；Clock in / Break / Clock out 三状态
- 活跃工作段（WorkSegment）自动累计：上班或从休息回来 → stretch 开始；休息/下班 → stretch 结束并落一段记录
- **分类后置**：切休息、下班、Deep Focus 结束时都弹「这段主要在干什么」选择框（默认选中 Mixed，回车即确认；Deep Focus 结束后自动开新段继续计）；粗分类 WorkKind：🌀混合/统筹（默认）/ 📚研究写作 / 📮行政 / 🧑‍🏫教学 / 📦其他 / ❓待定
- **Deep Focus 降级为可选工具**（openDeepFocus）：只选时长（25/40/50/90/不限/自定义）+ 备注，不选类别；开始 = 软气泡+猫叫（永远轻），到点 = 跟随全局模式提醒；HUD 显示 🎯 剩余/已用时间；软气泡期间猫可拖拽（isSoftReminderActive 放行鼠标），只有普通强提醒锁定拖拽
- **上班轻快**（猫叫一声即开始）、**下班强仪式**（final 下班走强提醒动画 🌙收工卡片，之后猫睡觉；sleepAfterOverlay 标记控制 restoreCatWindowSize 后的状态）
...
- 从 idle 状态随机触发：走路(~10%)、玩耍(~5%)、追尾巴(~3%)、翻肚皮(~3%)、舔毛(~5%)
- 走路每次随机速度（2.5~8 pts/step）+ ease-in-out 加减速曲线
- 玩耍 ~15s 后自动恢复 idle，追尾巴 ~10s 后自动恢复，舔毛 ~20s 后自动恢复
